...
class CurriculumCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.num_timesteps - self.last_eval_step >= 500:
        ...
        if self.num_timesteps - self.last_advance_eval_step >= 1_000:
            self.last_advance_eval_step = self.num_timesteps
            if len(self.win_buffer) >= config.WIN_RATE_WINDOW // 2:
