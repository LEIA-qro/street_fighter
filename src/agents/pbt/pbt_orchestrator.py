...
def run_agent_worker(config_dict):
    ...
    env = SubprocVecEnv([SFv2_make_env(rank, version=config_dict["env_version"])])
    try:
    ...
    if checkpoint_dir and os.path.exists(os.path.join(checkpoint_dir, "vecnorm.pkl")):
        ...
        if hasattr(env, "obs_rms"):
            env.obs_rms.count = min(env.obs_rms.count, 5000.0)
    ...
    custom_objs = {"learning_rate": active_lr, "ent_coef": active_ent, "clip_range": active_clip}
    ...
    if checkpoint_dir and os.path.exists(os.path.join(checkpoint_dir, "model.zip")):
        ...
        model = PPO("MlpPolicy", env=env, learning_rate=active_lr, n_steps=FIXED_PARAMS["n_steps"], batch_size=FIXED_PARAMS["batch_size"],
                    ...
                    policy_kwargs=dict(net_arch=dict(pi=[512,512,256], vf=[512,512,256])), verbose=0, device="cuda")
    ...
    try:
        while True:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                model.save(os.path.join(temp_dir, "model"))
                env.save(os.path.join(temp_dir, "vecnorm.pkl"))
                from ray.train import Checkpoint
                final_checkpoint = Checkpoint.from_directory(temp_dir)
                win_rate = sum(callback.win_buffer) / len(callback.win_buffer) if len(callback.win_buffer) > 0 else 0.0
                ray_train.report(metrics={"win_rate": win_rate, "timesteps": model.num_timesteps, "rank": rank}, checkpoint=final_checkpoint)
    finally:
        failsafe_env(env=env, model=model)

class PBTOrchestrator:
    def run(self, algo, env_version, total_steps, population_size, steps_per_exploit=STEPS_PER_EXPLOIT, start_phase="0", base_zip=None, base_pkl=None, model_name="PBT_BEST_model", resume=False):
        import ray
        ...
        pb2_scheduler = PB2(time_attr="timesteps", metric="win_rate", mode="max", perturbation_interval=steps_per_exploit, hyperparam_bounds=PB2_HYPERPARAM_SPACE)
        ...
        if resume and tune.Tuner.can_restore(exp_dir):
            ...
            tuner = tune.Tuner(
                ...
                tune_config=tune.TuneConfig(scheduler=pb2_scheduler, num_samples=1, metric="win_rate", mode="max"),
                run_config=ray_train.RunConfig(
                    name="pbt_sf2", storage_path=os.path.join(directories["tuning"], "pbt"), stop={"timesteps": total_steps},
                    checkpoint_config=ray_train.CheckpointConfig(num_to_keep=3, checkpoint_score_attribute="win_rate", checkpoint_score_order="max")
                )
            )
