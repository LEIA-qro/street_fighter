...
## [2026-08-26 noche-2] CORRECCIÓN al expediente W&B — causa raíz verificada

El asesino principal NO era (solo) el sidecar: **`run.log(metrics, step=N)` con step explícito deja la fila PARCIAL hasta que un log con step mayor la comete**. Cada reinicio del coordinator mataba el proceso con su única fila sin publicar — por eso "no había charts" en días de muchos despliegues y por eso la historia "aparecía" post-mortem (finish() comete parciales). Verificado: con el proceso estable, las filas fluyen en vivo con retraso de UNA generación (2 filas en servidor a gen 7 de run 3). La tormenta de analytics OTEL del wandb 0.28.2 era real pero secundaria (downgrade a 0.21.4 la redujo). Moral: el coordinator debe VIVIR; los reinicios frecuentes son el enemigo de las charts, no wandb per se. metrics.jsonl→S3 cubre cualquier hueco.
