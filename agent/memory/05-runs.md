...
## [2026-08-27 ~17:00] Evaluación definitiva del v1212 — 85.3% del juego COMPLETO

Banco canónico × 8 niveles × 3 condiciones (2,592 eps) sobre el campeón v1212 (media selector 85.4). Promedios por nivel: **100 / 99.7 / 98.3 / 80.2 / 84.7 / 83.7 / 72.2 / 63.9 → media global 85.3%**. Cero olvido (lvl1-3 en 98-100), lvl5-6 +18/+22 pts desde la mañana, lvl7 72% y lvl8 64% en su primera lectura completa (ayer: 25 y 10). Robustez viaja en toda la matriz salvo lvl8 (limpio 83 vs desfase 56: coreografía residual en los arranques del HARD — el desfase es el número honesto). Anomalía consistente: lvl4 (80.2) ≤ lvl5-6 (~84) — el tier 4 del ROM pega como 5-6. Acta: benchmarks/bench_v1212_total.jsonl; congelado: apex_v1212_escalera854.pt.

## [2026-08-27 ~18:00] Corte vespertino día 2 — la flota récord y las sagas de red

- **Flota récord: 4 máquinas / ~136 procesos / ~8k st/s** — la Omen entró DESDE CERO (runbook completo `tools/RUN_OMEN_DESDE_CERO.md`: WSL → tailscale-en-Ubuntu → venv actor-mínimo con torch CPU → ROM por romshare con nombre simple → actor). Su lección: si el nombre tailnet da "connection refused" con el learner sano, probar la IP directa (100.121.228.49) — puede estar navegando de prestado por el tailscale de Windows.
- **La saga de red de la Legion** (mudanza de laps): tailscale directo 34ms + GET/descargas de 5MB perfectas + TODO POST grande en "timed out" = NAT de WSL2 atorado; lo curó un REBOOT de Windows completo (los speed tests del navegador NO prueban esta ruta). De pilón: flag nuevo `--flush N` en apex_actor (lotes chicos para redes que matan subidas grandes).
